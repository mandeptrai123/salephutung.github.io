...
function LichSuGiaoDich() {
    ...
    const {DateTimKiem,TenTimKiem,SanPhamTimKiem,ValueTen,ValueSanPham} = state;
   ...
   function FectchToanBoSanPham()
    ...
    function FindItemBySanPham(nameSP)
    {
        ...
        {
            if(_arrDonHang[j].lstSanPham == undefined)
                return;
    ...
    function FindItemByTenKhach(nameTenKhach)
    {
        ...
        for(var j=0;j<_arrDonHang.length;j++)
        {
            if(_arrDonHang[j].TenKhach == undefined)
                return;
    ...
    useEffect(() => {
        ...
        var _date = new Date();
        var _dateDefault = _date.getFullYear()+"-"+((_date.getMonth()+1)>9?(_date.getMonth()+1):"0"+(_date.getMonth()+1))+"-"+(_date.getDate()>9?_date.getDate():"0"+_date.getDate());
    ...
    const [stateModal,setStateModal] = useState({open:false,itemSelected:null});
        ...
        >
            ...
            />
                ...
                <Autocomplete
                    ...
                    value={TenTimKiem}
                    onChange={e=>{ 
                        setState({...state,TenTimKiem:e.target.value})
                    }}
                    ...
                    style={{height:50,width:200,marginLeft:20,fontSize:14,paddingLeft:5}}
