...
function KhoHang() {
    ...
    function Hanlde_TimSanPham()
    {
        ...
        const _result  =  arr.map((e)=>{
            return <Result_row_find 
            name={e.Name}
            donvi={e.DonVi}
            giaban={e.GiaBan}
            soluong={e.SoLuong}
            txtBtn='Chỉnh sửa' titleDialog='Cập Nhật Công Nợ' style_Dialog_KhoHang={style_Dialog_KhoHang}/>
        })
    ...
    return(
        <section className='khohang-container'>
            <div className='khohang-container__content'>
                ...
                <input style={{width:400,height:50,marginLeft:100,padding:10}} type="text" name="name" placeholder="Nhập Tên Sản Phẩm"/>
                <Button onClick={e=>Hanlde_TimSanPham()} variant="primary" style={{height:50,marginLeft:20,marginTop:15}}>Tìm</Button>
                </div>
